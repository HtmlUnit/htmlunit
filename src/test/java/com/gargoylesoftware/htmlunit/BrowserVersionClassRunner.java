    ...
    private void setAlerts(final WebTestCase testCase, final Method method) {
        ...
        if (alerts != null) {
            if (isDefined(alerts.value())) {
                ...
                else if (browserVersion_ == BrowserVersion.FIREFOX_8) {
                    expectedAlerts = firstDefined(alerts.FF8(), alerts.FF(), alerts.DEFAULT());
                }
    ...
    private boolean isDefinedIn(final Browser[] browsers) {
        for (final Browser browser : browsers) {
            switch(browser) {
                ...
                case FF8:
                    if (browserVersion_ == BrowserVersion.FIREFOX_8) {
                        return true;
                    }
                    break;
