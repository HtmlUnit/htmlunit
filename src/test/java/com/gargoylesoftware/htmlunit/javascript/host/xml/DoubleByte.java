...
public class DoubleByte {
    ...
    public static class Decoder extends CharsetDecoder
    {
        ...
        protected CoderResult decodeArrayLoop(ByteBuffer src, CharBuffer dst) {
            byte[] sa = src.array();
            System.out.println("decodeArrayLoop " + sa.length + " " + src.arrayOffset() + " " + src.position() + " " + src.limit());
            int sp = src.arrayOffset() + src.position();
            int sl = src.arrayOffset() + src.limit();
            char[] da = dst.array();
            int dp = dst.arrayOffset() + dst.position();
            int dl = dst.arrayOffset() + dst.limit();
            System.out.println("decodeArrayLoop " + da.length + " " + dst.arrayOffset() + " " + dst.position() + " " + dst.limit());
            ...
            try {
                while (sp < sl && dp < dl) {
                    ...
                    int b1 = sa[sp] & 0xff;
                    char c = b2cSB[b1];
                    System.out.println("decodeArrayLoop c " + (int) b1 + " " + (int) c);
