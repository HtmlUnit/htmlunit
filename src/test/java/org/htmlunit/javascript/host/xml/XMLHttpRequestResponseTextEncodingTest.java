...
public class XMLHttpRequestResponseTextEncodingTest extends AbstractXMLHttpRequestEncodingTest {
    ...
    private void responseText(
        ...
        if (BOM_UTF_8.equals(bom)) {
            final byte[] xmlBytes = ArrayUtils.addAll(ByteOrderMark.UTF_8.getBytes(), xml.getBytes(StandardCharsets.UTF_8));
            getMockWebConnection().setResponse(URL_SECOND, xmlBytes, 200, "OK", mimeTypeXml.getMimeType(), null);
        }
        else if (BOM_UTF_16BE.equals(bom)) {
            final byte[] xmlBytes = ArrayUtils.addAll(ByteOrderMark.UTF_16BE.getBytes(), xml.getBytes(StandardCharsets.UTF_16BE));
            getMockWebConnection().setResponse(URL_SECOND, xmlBytes, 200, "OK", mimeTypeXml.getMimeType(), null);
        }
        else if (BOM_UTF_16LE.equals(bom)) {
            final byte[] xmlBytes = ArrayUtils.addAll(ByteOrderMark.UTF_16LE.getBytes(), xml.getBytes(StandardCharsets.UTF_16LE));
            getMockWebConnection().setResponse(URL_SECOND, xmlBytes, 200, "OK", mimeTypeXml.getMimeType(), null);
        }
        else {
