...
public class HttpWebConnection implements WebConnection {
    ...
    private void setProxy(final SimpleHttpRequest httpRequest, final WebRequest webRequest, final HttpAsyncClientBuilder clientBuilder) {
        ...
        if (webRequest.isSocksProxy()) {
            ...
            configBuilder.setSocksProxyAddress(new InetSocketAddress(webRequest.getProxyHost(), webRequest.getProxyPort()));
     ...
     */
    private SimpleHttpRequest makeHttpMethod(final WebRequest webRequest, final HttpAsyncClientBuilder httpClientBuilder)
        throws URISyntaxException {
        ...
        if (StringUtils.equalsAny(httpMethod.getMethod(), Method.POST.name(), Method.PUT.name(), Method.PATCH.name())) {
            ...
            if (webRequest.getEncodingType() == FormEncodingType.URL_ENCODED && httpMethod.getMethod().equals(Method.POST.name())) {
                if (webRequest.getRequestBody() == null) {
                    final List<NameValuePair> pairs = webRequest.getRequestParameters();
                    final String query = WWWFormCodec.format(NameValuePair.toHttpClient(pairs), charset);
            ...
            else if (webRequest.getEncodingType() == FormEncodingType.TEXT_PLAIN && httpMethod.getMethod().equals(Method.POST.name())) {
                if (webRequest.getRequestBody() == null) {
     ...
     */
    protected PoolingAsyncClientConnectionManagerBuilder createConnectionManagerBuilder(final int timeout, final WebClientOptions options) {
...
//        }
        ...
        if (connectionManager_ == null) {
            connectionManager_ = createConnectionManagerBuilder(timeout, options).build();
        }
...
//        }
    ...
    private static <T> void setField(final Object target, final String fieldName, T value) throws IllegalAccessException {
